...
class LLMConfig:
    """LLM配置管理类，优先从环境变量(.env文件)读取，支持OpenRouter多模型"""

    @staticmethod
    def load_env_config():
        """加载环境变量配置（兼容旧变量，但推荐使用 OPENROUTER_ 前缀）"""
        config = {
            "api_key": os.getenv("OPENROUTER_API_KEY") or os.getenv("AI_API_KEY"),
            "base_url": os.getenv("OPENROUTER_BASE_URL") or os.getenv("AI_API_BASE", "https://openrouter.ai/api/v1"),
            "default_model": os.getenv("DEFAULT_MODEL") or os.getenv("AI_MODEL", "openai/gpt-4o-mini"),
            "source": "环境变量(.env文件)"
        }
        return config
    ...
    def get_llm():
        ...
        try:
            ...
            print(f"✅ 默认LLM实例创建成功，模型: {model}")
            if base_url:
                print(f"🌐 API地址: {base_url}")
            return llm
        except Exception as e:
            print(f"❌ 默认LLM配置错误: {str(e)[:100]}")
            return None

    @staticmethod
    def get_collaboration_llm():
        """获取协作整合用的LLM实例（单独配置，此处复用默认配置）"""
        return LLMConfig.get_llm()

    @staticmethod
    def get_agent_llm(agent_name: str, default_llm=None):
        """
        获取特定智能体专用的LLM实例，所有请求都通过 OpenRouter 发送。
        agent_name: 如 'initiation', 'planning', 'resource', 'coaching', 'evaluation'
        default_llm: 如果配置失败则返回此默认LLM；若为None则尝试用全局配置创建新实例。
        """
        # 1. 获取全局 OpenRouter 配置
        env_config = LLMConfig.load_env_config()
        api_key = env_config["api_key"]
        base_url = env_config["base_url"]
        ...
        if not api_key:
            print("⚠️ 未找到 OpenRouter API Key，请检查 OPENROUTER_API_KEY 环境变量")
            if default_llm:
                return default_llm
            else:
                return LLMConfig.get_llm()  # 可能返回None

        # 2. 获取该智能体的专用模型名称
        model_env = f"AGENT_{agent_name.upper()}_MODEL"
        ...
        try:
            ...
            print(f"✅ 为智能体 {agent_name} 创建 OpenRouter 实例，模型: {model}")
            return llm
        except Exception as e:
            print(f"❌ 为智能体 {agent_name} 创建 LLM 失败: {e}")
            if default_llm:
                return default_llm
            # 最后尝试用全局配置创建（可能失败）
            return ChatOpenAI(
...
class PBLAgents:
    ...
    # [修改] 新增方法：获取指定智能体专用的LLM
    def _get_agent_llm(self, agent_name):
        return LLMConfig.get_agent_llm(agent_name, self.default_llm)
    ...
    def _create_agent_chains(self):
        ...
        self.initiation_chain = (
                ...
                | self._get_agent_llm('initiation')   # [修改]
                | StrOutputParser()
        )
        ...
        self.planning_chain = (
                ...
                | self._get_agent_llm('planning')     # [修改]
                | StrOutputParser()
        )
        ...
        self.resource_chain = (
                ...
                | self._get_agent_llm('resource')     # [修改]
                | StrOutputParser()
        )
        ...
        self.coaching_chain = (
                ...
                | self._get_agent_llm('coaching')     # [修改]
                | StrOutputParser()
        )
        ...
        self.evaluation_chain = (
                ...
                | self._get_agent_llm('evaluation')   # [修改]
                | StrOutputParser()
        )
